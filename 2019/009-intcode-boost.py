...
def get_from_buff(buff, val, mode, rel_base):
    if mode == 0:
        return buff[val]
    elif mode == 1:
        return val
        ...
        # Use relative base
        return buff[rel_base + val]
    else:
        raise ValueError("Unexpected Parameter Mode: {0}".format(mode))


def take_step(buff, idx=0, input_buffer=None, rel_base=0):
    op_code = buff[idx]
    ...
    if instruction == 99:
        return buff, idx + 1, True, None, output, False, rel_base
    elif instruction in (1, 2, 7, 8):
        arg1 = get_from_buff(buff, buff[idx + 1], p1_mode, rel_base)
        arg2 = get_from_buff(buff, buff[idx + 2], p2_mode, rel_base)
        if instruction == 1:
            # add
            res = arg1 + arg2
        elif instruction == 2:
        ...
        if input_buffer is not None:
            ...
            return buff, idx, False, None, output, True, rel_base
        i = int(i)
        buff[buff[idx + 1]] = i
        idx += 2
    elif instruction == 4:
    ...
    return buff, idx, False, None, output, False, rel_base
...
class IntComp(object):
    ...
    def __init__(self, code=None):
        self.code = code or self.init_code
        # extend memory to full size
        self.code += [0] * (self.memory_size - len(self.code))
        self.halt = False
        self.prog_pointer = 0
    ...
    def run(self, input_buffer=None):
        if not self.halt:
            while True:
                self.code, self.prog_pointer, self.halt, input_buffer, output, self.await_input, self.rel_base = take_step(
                    self.code, idx=self.prog_pointer, input_buffer=input_buffer, rel_base=self.rel_base)
                if output:
                    return output
                if self.halt:
                    break
