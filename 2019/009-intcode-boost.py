...
class IntComp(object):
    ...
    def take_step(self, buff, idx=0, input_buffer=None):
        op_code = buff[idx]
        ...
        if instruction == 99:
            ...
            arg1 = get_from_buff(buff, buff[idx + 1], p1_mode, self.rel_base)
            arg2 = get_from_buff(buff, buff[idx + 2], p2_mode, self.rel_base)
            if instruction == 1:
                # add
                res = arg1 + arg2
            elif instruction == 2:
    ...
    def run(self, input_buffer=None):
        if not self.halt:
            while True:
                self.code, self.prog_pointer, self.halt, input_buffer, output, self.await_input= self.take_step(
