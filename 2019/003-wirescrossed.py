...
class LineSeg(object):
    __slots__ = ['x', 'y', 'ln', 'd', 'cd']

    def __init__(self, x, y, ln, d, cd):
        self.x = x
        self.y = y
        self.ln = ln
        d = d.lower()
    ...
    def get_end(self):
        if self.d == 'u':
            return (self.x, self.y + self.ln)
        elif self.d == 'd':
            return (self.x, self.y - self.ln)
        elif self.d == 'l':
            return (self.x - self.ln, self.y)
        elif self.d == 'r':
            return (self.x + self.ln, self.y)
        else:
    ...
    def __str__(self):
        return "<LineSeg: {s!r} {d}:{l}>".format(
            s=self.start, d=self.d, ln=self.ln)
    ...
    def is_between(a, b1, b2):
        ...
        if b1 == b2:
            ...
            raise ValueError("Unexpected is_between, {a}, {b}, {c}".format(
                **locals()))
    ...
    def insersects(self, other):
        ...
        if self.orientation == 'v':
            ...
            if self.is_between(self.start[0], other.start[0], other.end[0]):
                # AND it means that for other the y is the same at the
                # start and end
...
def compare(l1, l2):
    ib = LineSeg.list_intersections(
        LineSeg.list_from_raw(l1), LineSeg.list_from_raw(l2)
    )
    print(ib)
